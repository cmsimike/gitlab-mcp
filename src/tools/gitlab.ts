...
function getGitLabToolDefinitions(): GitLabToolDefinition[] {
  return [
    ...
    },
    {
      ...
      description:
        "Save one file from a job artifacts archive to a local directory. Set inline=true for inline content; prefer gitlab_get_job_artifact_file_inline for new clients.",
      mutating: true,
      requiresFeature: "pipeline",
      requiresLocalFileTools: true,
      inputSchema: {
      ...
      handler: async (args, context) => {
        ...
        if (getOptionalBoolean(args, "inline")) {
