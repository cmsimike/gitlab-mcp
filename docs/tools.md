...
| `gitlab_download_job_artifacts`       | **Yes**  | Download the full artifacts archive to a local directory. Supports `local_path`. Unavailable when `REMOTE_AUTHORIZATION=true`.                                                               |
| `gitlab_get_job_artifact_file`        | **Yes**  | Save one file from a job artifacts archive to a local directory. Supports `local_path`. Unavailable when `REMOTE_AUTHORIZATION=true`.                                                        |
