...
| `gitlab_get_job_artifact_file_inline` | No       | Return one file from a job artifacts archive as inline content. Text-like files are UTF-8; binary files are base64.                                                                                                                                   |
| `gitlab_download_job_artifacts`       | **Yes**  | Download the full artifacts archive to a local directory. Supports `local_path`. Available only on local transports; not exposed over HTTP.                                                                                                           |
| `gitlab_get_job_artifact_file`        | **Yes**  | Save one file from a job artifacts archive to a local directory. Supports `local_path` and `inline=true` for compatibility; prefer `gitlab_get_job_artifact_file_inline` for inline reads. Available only on local transports; not exposed over HTTP. |
